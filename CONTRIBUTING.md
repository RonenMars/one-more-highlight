...
**If a release ever fails at `git push` with `GH013: Repository rule violations found for refs/heads/main`, look here first.** (Rulesets report `GH013`; the older `GH006` comes from classic branch protection. The rejection lists both "Changes must be made through a pull request" and "N of N required status checks are expected" — required status checks gate a direct push, not only a merge, whatever the GitHub docs' merge-centric wording suggests.) semantic-release runs on `secrets.GITHUB_TOKEN`, so it pushes its `chore(release)` commit as `github-actions[bot]`, which the repo-admin bypass does not cover — and GitHub refuses to add the Actions app as a bypass actor on a personal repository. The failure is loud and lands in `@semantic-release/git`'s prepare step, *before* the npm publish, so nothing is ever half-released. That is exactly what is configured: `RELEASE_PAT` is a fine-grained PAT owned by a repo admin, and `.github/workflows/ci.yml` reads `GITHUB_TOKEN: ${{ secrets.RELEASE_PAT || secrets.GITHUB_TOKEN }}`.

**It expires on 2027-09-05, and releases will fail with the same `GH013` on that day.** To rotate it, create a fine-grained PAT scoped to this repository with **Contents**, **Issues** and **Pull requests** all set to read/write, then `gh secret set RELEASE_PAT --repo RonenMars/one-more-highlight` and re-run the failed job.
The last two permissions are not optional: the workflow's `permissions:` block governs `GITHUB_TOKEN`, not a PAT, so with `Contents` alone the release publishes to npm and *then* fails in `@semantic-release/github`'s `success` step — the one failure mode here that leaves the registry ahead of the repository.
